const scenarios = [
  ...
  {
    id: 'malware-3',
    title: 'The Ransomware Payload',
    category: 'Malware',
    difficulty: 2,
    description: 'Your workstation suddenly displays a countdown for file encryption.',
    icon: 'Bug',
    steps: [
      {
        prompt: 'You were cleaning up your drive when you accidentally ran a file hidden in the temp folder. Now, your screen turns red with a timer: "ALL YOUR FILES HAVE BEEN ENCRYPTED! Send 0.5 BTC to this address within 24 hours or they will be deleted forever."',
        visualType: 'popup',
        visualData: {
          title: 'RANSOMWARE ALERT',
          message: 'Files Encrypted! Payment Required: 0.5 BTC',
          buttonText: 'HOW TO PAY',
          isFlashing: true
        },
        options: [
          {
            text: 'Disconnect the network cable immediately',
            isCorrect: true,
            feedback: '✅ Good! Fast isolation can stop the malware from spreading to servers or encrypting network drives.',
            defenseTip: 'If you suspect ransomware, disconnect from the network ASAP to contain the damage.'
          },
          {
            text: 'Pay the ransom quickly to get my files back',
            isCorrect: false,
            feedback: '❌ Paying ransoms only funds more attacks and doesn\'t guarantee you\'ll get your files back.',
            defenseTip: 'Never pay ransoms. It invites more attacks and rarely leads to data recovery. Rely on backups.'
          }
        ]
      }
    ]
  },
  {
    ...
    difficulty: 3,
    timeLimit: 30,
    description: 'The final confrontation with the ransomware master node.',
    icon: 'Shield',
    steps: [
      {
        id: 'start',
        prompt: 'You\'ve reached the heart of Black Byte\'s operation. The master node is isolated, but they\'ve triggered a recursive encryption sequence. You have seconds to act. A prompt appears on the console: "DECRYPTION KEY OVERRIDE: SECTOR 0 - Enter auth bypass protocol."',
        visualType: 'popup',
        visualData: {
          title: 'SYSTEM LOCKDOWN IN PROGRESS',
          message: 'Encryption level: AES-256\nProgress: 88%\nCounter-measures: ACTIVE',
          buttonText: 'BYPASS FIREWALL',
          isFlashing: true
        },
        options: [
          {
            text: 'Execute emergency kernel shutdown',
            isCorrect: true,
            feedback: '✅ Direct action! The shutdown signal halted the encryption at 92%. Now you need to find the kill-command.',
            nextStepId: 'kill-command'
          },
          {
            text: 'Attempt to reverse the encryption live',
            isCorrect: false,
            feedback: '❌ The encryption is too fast. It fed on your processing cycles. Sector 0 has been encrypted. Mission failure.',
            isTerminal: true
          }
        ]
      },
      {
        id: 'kill-command',
        prompt: 'The system is stalling. A hidden directory has appeared: "/root/blackbyte/payloads". You see a file named "logic_bomb.sh". How do you handle it?',
        visualType: 'file-explorer',
        visualData: {
          driveName: 'B-BYTE-CORE',
          drivePath: '/root/blackbyte/payloads',
          files: [
            { name: 'logic_bomb.sh', size: '2KB', isSuspicious: true },
            { name: 'config.json', size: '4KB' },
            { name: 'keys.db', size: '1MB' }
          ]
        },
        options: [
          {
            text: 'Delete logic_bomb.sh immediately',
            isCorrect: true,
            feedback: '✅ The payload is neutralized! The malware cannot execute its final stage.',
            nextStepId: 'final-choice'
          },
          {
            text: 'Try to open keys.db to recover user data',
            isCorrect: false,
            feedback: '❌ It was a trap! Accessing the keys.db triggered a self-destruct sequence. The server has wiped itself.',
            isTerminal: true
          }
        ]
      },
      {
        id: 'final-choice',
        prompt: 'One final move. You have access to the master server\'s uplink. Do you disconnect the network entirely or attempt to trace the physical location of the hackers?',
        visualType: 'decision',
        options: [
          {
            text: 'Disconnect the master node from the internet',
            isCorrect: true,
            feedback: '✅ MISSION ACCOMPLISHED! Black Byte is offline. You have saved the city\'s infrastructure.',
            defenseTip: 'In a live ransomware attack, isolation is the priority. Stop the spread, then investigate.'
          },
          {
            text: 'Keep the connection open to trace the attackers',
            isCorrect: false,
            feedback: '❌ They were faster than you. While you were tracing them, they used a hidden backup line to finish the encryption. You lost.',
            isTerminal: true
          }
        ]
      }
